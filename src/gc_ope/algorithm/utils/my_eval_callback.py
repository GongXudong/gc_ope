...
class MyEvalCallbackSTAT(EventCallback):
    """
    Callback for evaluating an agent.
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, success_rate, stat_dict_arr = evaluate_policy_with_stat(
                self.model,
                self.eval_env,
                n_eval_episodes=self.n_eval_episodes,
                render=self.render,
...
class MyEvalCallbackSTATFromReplayBuffer(MyEvalCallbackSTAT):
    """
    Callback for evaluating an agent.
    ...
    def __init__(
        ...
        verbose: int = 1,
        warn: bool = True,
        reset_evaluation_result_container: bool = True,
        reset_evaluation_result_container_method_name: str = "reset_evaluation_result_container",
        sample_num_from_replay_buffer: int = 1000,
        sync_success_stat: bool = True,  # 向环境发送评估信息
        sync_success_stat_env_method_name: str = "sync_evaluation_stat",
        retrieve_info_from_env: bool = False,  # 从环境提取信息，用于sb3.logger记录
        retrieve_info_from_env_method_name: str = "get_info_to_log",
        ...
        super().__init__(
            ...
            success_key_in_info=success_key_in_info,
            render=render,
            verbose=verbose,
            warn=warn,
            sync_success_stat=sync_success_stat,
            sync_success_stat_env_method_name=sync_success_stat_env_method_name,
            retrieve_info_from_env=retrieve_info_from_env,
            retrieve_info_from_env_method_name=retrieve_info_from_env_method_name,
            training_envs=training_envs,
        )

        # 是否重置估计p_ag使用的数据容器
        self.reset_evaluation_result_container = reset_evaluation_result_container
        self.reset_evaluation_result_container_method_name = reset_evaluation_result_container_method_name
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # 重置env端的评估数据容器
            if self.reset_evaluation_result_container:
                self.training_envs.env_method(self.reset_evaluation_result_container_method_name)

            # 从replay buffer中随机选取数据
            replay_buffer = self.locals["self"].replay_buffer
            ...
            random_index = np.random.randint(
                ...
                size=self.sample_num_from_replay_buffer,
            )
