...
def test_get_all_possible_dgs_and_dV():
    ...
    all_dgs, dV = my_reach_desired_goal_utils.get_all_possible_dgs_and_dV(env)
    print(len(all_dgs), dV, np.array(all_dgs))

    all_dgs, dV = my_reach_desired_goal_utils.get_all_possible_dgs_and_dV(env, step_list=[0.02, 0.02, 0.02])
    print(len(all_dgs), dV, np.array(all_dgs))
