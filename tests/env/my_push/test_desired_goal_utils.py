...
from gc_ope.env.utils.my_push import desired_goal_utils as my_push_desired_goal_utils
...
def test_sample_a_desired_goal(test_pkg: Literal["my_push", "common"]):
    print("In sample a desired goal:")

    env = gym.make("MyPushSparse-v0")

    if test_pkg == "my_push":
        dgs = [my_push_desired_goal_utils.sample_a_desired_goal(env) for i in range(100)]
    else:
...
def test_get_all_possible_dgs():
    print("In test get all possible dgs:")
...
def test_get_random_dgs():
    ...
    env = gym.make("MyPushSparse-v0")
    random_dgs = my_push_desired_goal_utils.get_random_dgs(env, 10)
...
def test_reset_env_with_desired_goal(test_pkg: Literal["my_push", "common"]):
    ...
    if test_pkg == "my_push":
        dg_list = [my_push_desired_goal_utils.sample_a_desired_goal(env) for i in range(100)]
    else:
        dg_list = [common_desired_goal_utils.sample_a_desired_goal(env) for i in range(100)]

    for dg in dg_list:
        if test_pkg == "my_push":
            obs, info = my_push_desired_goal_utils.reset_env_with_desired_goal(env, dg)
        else:
