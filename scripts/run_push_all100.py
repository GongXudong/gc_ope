    ...
    try:
        ...
        while len(finished) < len(processes):
            for seed, process in processes.items():
                ...
                if audit["fit_warnings"]:
                    report(f"{method} seed={seed}：{audit['fit_warnings']} 个 checkpoint 有拟合质量提醒，见 CSV 的 fit_warnings")
...
def main():
    ...
    methods = [method for method in AVAILABLE_METHODS if method in args.methods]
    ...
    manifest = {"methods": methods, "seeds": args.seeds, "checkpoints": args.checkpoints,
